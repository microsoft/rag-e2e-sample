...
---
In this sub-directory, the deployment workflow for a chatbot or web app demonstrates answer generation based on search on documents. It demonstrates how to deploy a streamlit-chatbot (LLM-powered) User Interface in Azure for demo purposes. This deployment workflow is in python and uses [Streamlit](https://streamlit.io/) + [Azure Container Registry (ACR)](https://azure.microsoft.com/en-us/products/container-registry) + [Azure Web App](https://azure.microsoft.com/en-us/products/app-service/containers?activetab=pivot:deploytab) to deploy a chat interface.
...
4. Add the `.env` file with config variables for access
5. Configure your PostGres Flex server to be accessible from your laptop if running locally or from a webapp if running from Azure. Instructions [here](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/tutorial-webapp-server-vnet)
...
2. The streamlit app is defined in a folder `streamlit_app/`  
   ...
   2. `pg_qna.py` contain the chatbot response logic.
   3. `config.yml` contains all configurations such as title, logos etc.
3. `Dockerfile` and `environment.yml` define how the container should be built
4. `llm_pgvector.env` should be added manually or update `example.env` and rename it as `llm_pgvector.env`
...
2. **To deploy to Azure web app as a Docker container**
   ...
   3. Create a web app using the built container `az webapp create --resource-group <RG-NAME> --plan <APP-SERVICE-PLAN-NAME> --name <WEBAPP-NAME> -i <ACR-NAME>.azurecr.io/bot:latest`
   4. Configure your web app to listen to port 8000 and set a longer container_start_time_limit `az webapp config appsettings set --resource-group <RG-NAME> --name <WEBAPP-NAME> --settings WEBSITES_PORT=8000 WEBSITES_CONTAINER_START_TIME_LIMIT=1800 `
   5. Open a browser at `<WEBAPP-NAME>.azurewebsites.net` (note: it may take a few minutes to load the first time the container is started)
...
** Note: If you update the docker image, you can simply restart the webapp.
...
2. Re-create a web app `az webapp create -g <RG-NAME> -p <APP-SERVICE-PLAN-NAME> -n <WEBAPP-NAME> -i <ACR-NAME>.azurecr.io/bot:latest `
3. Note: sometimes the webapp needs to be manually stopped and started again to rebase the backend with the latest container
`az webapp stop --resource-group <RG-NAME> --name <WEBAPP-NAME>`
`az webapp start --resource-group <RG-NAME> --name <WEBAPP-NAME>`
4. Container logs can be handy fro troubleshooting, link on how to access [these](https://stackoverflow.com/questions/52245077/where-can-i-find-docker-container-logs-for-azure-app-service) in the web app on portal.azure.com (look for `log stream on the left`)